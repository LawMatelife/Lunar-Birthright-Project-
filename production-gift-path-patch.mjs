...
function heroAndPaths(){
  ...
  if(free){free.textContent='Claim Mine Free';free.classList.add('lbp-free-path')}
  var premium=byText('button,a,[role="button"]',/(create a personalised gift|personalised gift|gift.*nz\\$?12|nz\\$?12.*gift)/i);
  if(premium){premium.textContent='Create a Personalised Gift — NZ$12';premium.classList.add('lbp-premium-path')}

  if(free&&!premium&&!document.getElementById('lbp-premium-path')){
    premium=document.createElement('a');premium.id='lbp-premium-path';premium.href='/gift/';premium.className='lbp-premium-path';premium.textContent='Create a Personalised Gift — NZ$12';free.insertAdjacentElement('afterend',premium);
  }
...
function run(){removeInternalStatus();heroAndPaths();enforceTokenVisibility();certificateAndCheckoutDisclosure()}
